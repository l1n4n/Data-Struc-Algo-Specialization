...
def solve(text, n, patterns):
    ...
    for pattern in patterns:
        ...
        for i, c in enumerate(pattern):
            ...
            if i == len(pattern) - 1:
                currentNode.next[c].patternEnd = True
            else:
    ...
    for i in range(len(text)): # iterate len(text) times
        ...
        while index < len(text): 
            ...
            if currentNode.patternEnd: # if the edge's end is a leaf, i.e., all former letters match, then the whole pattern is matched
                result.append(i)
                break
